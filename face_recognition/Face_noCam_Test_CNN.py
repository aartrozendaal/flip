...
picam2 = Picamera2()
picam2.configure(picam2.create_preview_configuration(main={"size": (640, 480)}))  # HD resolution
picam2.start()
...
loop_times = []
num_loops = 10 # number of loops performed
...
def get_closest_face_coordinates():
    ...
    # Detect faces
    detections = cnn_detector(img, 1)  # 1 = upsample once for better detection

    global count_exactly_x_faces
    if len(detections) == number_of_faces:
    ...
    if len(detections) > 0:
        ...
        for det in detections:
            rect = det.rect  # Get the rectangle from the detection
    ...
    return largest_face_coords, len(detections)
